package com.dyg.siginprint.CutInventory.view;
...
import com.dyg.siginprint.CutInventory.model.CutCacheBean;
...
public class CutInventoryActivity extends BaseActivity<PurchasePresenter> implements IPurchaseView , ClearEditText.Listener {
    ...
    private ClearEditText cutBillEt;//分切单
    ...
    private int CODE_SCAN_BILLNO = 100;//分切单
    ...
    protected int attachLayoutRes() {
        return R.layout.activity_cut_inventory;
    }

    @Override
    protected void initPresenter() {
        presenter = new PurchasePresenter(CutInventoryActivity.this,this);
        initTitle(true, "分切出入库", "", new View.OnClickListener() {
            @Override
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        if (loginBean == null) finish();
            ...
            cutBillEt.initEditText(0, "", 0, 2, true, this);
            ...
            //需要在表格初始化之后，使用缓存数据
            //取缓存数据
            storageCode = Hawk.get(userCode + HawkKeys.HAWK_cut_storageCode , null);
            storageName = Hawk.get(userCode + HawkKeys.HAWK_cut_storageName , null);
            StorageTv.setText(storageName);
            CutCacheBean temCutBean = Hawk.get(userCode + HawkKeys.HAWK_cut_temBean);
            if(temCutBean != null){
                ...
                cutBillEt.setTextCt(temCutBean.getSourceNo(),true);
    ...
    private void initHR(){
        ...
        //3.4 获取控件设定的最大宽度
        setMaxWidth = getSetMaxWidthList(CutInventoryActivity.this, R.layout.item_hrecycleview, mMoveViewListID);
    ...
    private void clearView(boolean clearDefuatCache){
        ...
        if(clearDefuatCache){
            ...
            storageName = "";
            storageCode = "";
            Hawk.delete(userCode + HawkKeys.HAWK_cut_storageName);
            Hawk.delete(userCode + HawkKeys.HAWK_cut_storageCode);
            StorageTv.setText("");
        }
        Hawk.delete(userCode + HawkKeys.HAWK_cut_temBean);
...
//                //供应商
                showLoadingDialog();
                presenter.requestBaseInfo("FormPur",-90);
            }
            break;
            ...
            case R.id.tv_save:{
                ...
                if(!DoubleClickU.isFastDoubleClick(R.id.tv_save)){
                    ...
                    try {
                        ...
                        joData.put("sourceNo" , cutBillEt.getTextCt());
                    ...
                    showLoadingDialog();
                    presenter.requestScanResulte( "FormCut",joData);
                }
            }
            break;
    ...
    public void confirm(int etId, String result) {
        if(etId == 1){
            ...
            //分切单扫码完成焦点切到条码
            cutBillEt.setTextCt(result,true);
            //设置焦点
            ...
            showLoadingDialog();
            presenter.requestAnalysisQRCode("FormCut",joData);
        }
    }
        ...
        ListDialog dialog = new ListDialog(mActivity, new ListDialog.Callback() {
            ...
            public void selectFinish(Object obj, int position) {
                ...
                if(code == -90){
                    ...
                    //仓库缓存
                    Hawk.put(userCode + HawkKeys.HAWK_cut_storageCode,storageCode);
                    Hawk.put(userCode + HawkKeys.HAWK_cut_storageName,storageName);
                }
            }
        });
        dialog.show(list,code == -90 ? "选择单据类型" : code == -23 ? "选择仓库" : "选择" , "name");
