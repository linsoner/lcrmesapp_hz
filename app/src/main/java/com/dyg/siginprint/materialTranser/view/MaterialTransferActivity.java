package com.dyg.siginprint.materialTranser.view;
...
public class MaterialTransferActivity extends BaseActivity<PurchasePresenter> implements IPurchaseView, ClearEditText.Listener {
...
//    private ArrayList<Integer> idealWidthList = new ArrayList<>();
    ...
    private TextView processTv;//工序
    private TextView customerCodeTv;//生产车间
    ...
    protected void initPresenter() {
        presenter = new PurchasePresenter(MaterialTransferActivity.this,this);
        initTitle(true, "材料调拨", "", new View.OnClickListener() {
            @Override
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        if (loginBean == null) finish();
            ...
            mActivity = MaterialTransferActivity.this;
            ...
            //需要在表格初始化之后，使用缓存数据
            //取缓存数据
            processName = Hawk.get(userCode + HawkKeys.HAWK_ProTransfer_ToCustomerName , null);
            processCode = Hawk.get(userCode + HawkKeys.HAWK_ProTransfer_ToCustomerCode , null);
...
//        idealWidthList = getIdealWidthList(maxWidth, setMaxWidth);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.toCustomerCodeId:{
                //调入客户
                showLoadingDialog();
                presenter.requestBaseInfo("FormMatDiaobo",-1);
            }
            break;
            case R.id.customerCodeId:{
                //调出客户
                showLoadingDialog();
                presenter.requestBaseInfo("FormMatDiaobo",4);
            }
            break;
            case R.id.toStorageId:{
                //调入仓库
                showLoadingDialog();
                presenter.requestBaseInfo("FormMatDiaobo",-23);
            }
            break;
            case R.id.storageId:{
                //调出仓库
                showLoadingDialog();
                presenter.requestBaseInfo("FormMatDiaobo",-23 + 100000);
            }
            break;
            ...
            case R.id.tv_save:{
                ...
                if(!DoubleClickU.isFastDoubleClick(R.id.tv_save)){
                    ...
                    try {
                        ...
                        joData.put("processCode", processCode);
                    ...
                    showLoadingDialog();
                    presenter.requestScanResulte("FormMatDiaobo",joData);
                }
            }
            break;
    ...
    public void confirm(int etId, String result) {
        if(etId == 1){
            ...
            if(customerCode == null || customerCode.equals("")){
                ToastUtil.show(mActivity,"请选择生产车间",true);
                return;
            }
            if(toStorageCode == null || toStorageCode.equals("")){
                ToastUtil.show(mActivity,"请选择调入仓库",true);
                return;
            }
            if(storageCode == null || storageCode.equals("")){
                ToastUtil.show(mActivity,"请选择调出仓库",true);
                return;
            }
            ...
            showLoadingDialog();
            presenter.requestAnalysisQRCode("FormMatDiaobo",joData);
        }
    }
    ...
    public void baseInfoSuccess(List list , final int code) {
        ...
        ListDialog dialog = new ListDialog(mActivity, new ListDialog.Callback() {
            ...
            public void selectFinish(Object obj, int position) {
                ...
                if(code == -1){
                    ...
                    processCode = bean.getCode();
                    processTv.setText(bean.getName());
                }else if(code == 4){
                    //生产车间
        ...
        dialog.show(list,code == -1 ? "选择工序" : (code == 4) ? "选择生产车间" : code == -23 ? "选择调入仓库" : (code == -23 + 100000) ? "选择调出仓库" : "" , "name");
