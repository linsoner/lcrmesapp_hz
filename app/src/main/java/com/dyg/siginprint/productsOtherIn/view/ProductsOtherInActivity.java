package com.dyg.siginprint.productsOtherIn.view;
...
import com.dyg.siginprint.productsOtherIn.model.ProductsOtherInBean;
...
public class ProductsOtherInActivity extends BaseActivity<PurchasePresenter> implements IPurchaseView , ClearEditText.Listener {
    ...
    private TextView customerCodeTv;//分切单
    ...
    protected int attachLayoutRes() {
        return R.layout.activity_products_other_in;
    }

    @Override
    protected void initPresenter() {
        presenter = new PurchasePresenter(ProductsOtherInActivity.this,this);
        String title = "成品其他入库";
        if(viewId.equals("13")) title = "成品其他出库";
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        if (loginBean == null) finish();
            ...
            mActivity = ProductsOtherInActivity.this;
    ...
    private void initHR(){
        ...
        //3.4 获取控件设定的最大宽度
        setMaxWidth = getSetMaxWidthList(ProductsOtherInActivity.this, R.layout.item_hrecycleview, mMoveViewListID);
    ...
    @OnClick({R.id.billTypeLayoutId,R.id.StorageLayout,R.id.tv_temporary_storage,R.id.customerCodeTvLayoutId,R.id.tv_clearing,R.id.tv_save,R.id.tv_deleteRow})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.billTypeLayoutId:{
//                //入库单型
                showLoadingDialog();
                if(viewId.equals("13")) presenter.requestBaseInfo("FormPur",-94);
                else presenter.requestBaseInfo("FormPur",-95);
            }
            break;
            case R.id.StorageLayout:{
                //成品仓库
                showLoadingDialog();
                presenter.requestBaseInfo("FormPur",-21);
            }
            break;
            case R.id.customerCodeTvLayoutId:{
                //车间
                showLoadingDialog();
                presenter.requestBaseInfo("FormPur",4);
            }
            break;
            case R.id.tv_temporary_storage:{
                ...
                showLoadingDialog();
                ProductsOtherInBean bean = new ProductsOtherInBean();
            ...
            case R.id.tv_save:{
                ...
                if(!DoubleClickU.isFastDoubleClick(R.id.tv_save)){
                    ...
                    showLoadingDialog();
                    if(viewId.equals("13"))
                        presenter.requestScanResulte( "FormProOut",joData);
                    else
                        presenter.requestScanResulte( "FormProIn",joData);
                }
            }
            break;
    ...
    public void baseInfoSuccess(List list , final int code) {
        ...
        String typeName = "选择";
        if(code == -21) typeName = "选择仓库";
        else if(code == 4) typeName = "选择车间";
        else if(code == -95 || code == -94) typeName = "选择单据类型";
